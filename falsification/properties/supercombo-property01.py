...
lb0_sig = np.clip(1 / (1+np.exp(-output[0, 5486])) - percent_margin, 0.001, 0.999)
lb0 = np.log(lb0_sig / (1-lb0_sig))
ub0_sig = np.clip(1 / (1+np.exp(-output[0, 5486])) + percent_margin, 0.001, 0.999)
ub0 = np.log(ub0_sig / (1-ub0_sig))
lb1_sig = np.clip(1 / (1+np.exp(-output[0, 5488])) - percent_margin, 0.001, 0.999)
lb1 = np.log(lb1_sig / (1-lb1_sig))
ub1_sig = np.clip(1 / (1+np.exp(-output[0, 5488])) + percent_margin, 0.001, 0.999)
...
Forall(
    ...
    Implies(
        ...
        And(lb0 <= N(x_, desire, traffic_convention, initial_state)[0, 5486] <= ub0,
            lb1 <= N(x_, desire, traffic_convention, initial_state)[0, 5488] <= ub1,
        )
    ),
)
